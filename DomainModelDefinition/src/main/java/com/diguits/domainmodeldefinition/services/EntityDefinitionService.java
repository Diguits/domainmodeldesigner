...
import com.diguits.domainmodeldefinition.serialization.services.ISerializerProvider;
...
public class EntityDefinitionService implements IEntityDefinitionService {
    ...
    public void serialize(String targetPath) throws FileNotFoundException,
        ...
        getEntityDefinitionSerializer(targetPath).serialize(targetPath, domainModelDef);
    }

    private IEntityDefinitionSerializer getEntityDefinitionSerializer(String targetPath) {
        String ext = "";
        int index = targetPath.lastIndexOf('.');
        if (index < targetPath.length() - 1) {
            ext = targetPath.substring(index + 1);
        }
        return serializerProvider.getSerializer(ext);
    }
    ...
    public void deserialize(String sourcePath) throws FileNotFoundException,
        ...
        setDomainModelDef(getEntityDefinitionSerializer(sourcePath).deserialize(sourcePath));
    }

    @Override
    public void serialize(OutputStream targetStream, String ext) throws IOException {
        if (targetStream == null)
            throw new IllegalArgumentException("targetStream");
        serializerProvider.getSerializer(ext).serialize(targetStream, domainModelDef);
    }

    @Override
    public void deserialize(InputStream sourceStream, String ext) throws IOException {
        if (sourceStream == null)
            throw new IllegalArgumentException("sourceStream");
        setDomainModelDef(serializerProvider.getSerializer(ext).deserialize(sourceStream));
    }
